...
prev_pan_angles = [0.0] * light_count
prev_pan_angles = np.array(prev_pan_angles)
...
def osc_swarm_target_response(address, *args):
    ...
    for light_nr in range(light_count):
       ...
       # target_pos = np.array( [-args[arg_index  + 1] * flock_to_light_pos_scale, -args[arg_index] * flock_to_light_pos_scale, -args[arg_index + 2] * flock_to_light_pos_scale])
        ...
        # test euler to quat to euler conversion
        ...
        # check if absolute difference in pan angle is greather than 180, if yes, add or subtract 360 degrees
        ...
        """
        if light_rot_degrees[0] < -170.0 and prev_pan_angles[light_nr] > 170.0:
            if debug_print == True and light_nr == debug_light_index:
                print("+ 360.0")
            light_rot_degrees[0] += 360.0
        elif light_rot_degrees[0] > 170.0 and prev_pan_angles[light_nr] < -170.0:
            if debug_print == True and light_nr == debug_light_index:
                print("- 360.0")
            light_rot_degrees[0] -= 360.0
        """
        ...
        """
        if light_rot_degrees[0] - prev_pan_angles[light_nr] < -180.0:
            if debug_print == True and light_nr == debug_light_index:
                print("< -180.0")
            light_rot_degrees[0] += 360.0
        elif light_rot_degrees[0] - prev_pan_angles[light_nr] > 180.0:
            if debug_print == True and light_nr == debug_light_index:
                print("> 180.0")
            light_rot_degrees[0] -= 360.0
        """
        ...
        prev_pan_angles[light_nr] = light_rot_degrees[0]
