...
class CustomEnvironment(Environment):
    ...
    def reset(self):
        ...
        # Initialize episode_reward as a vector for MORL
        self.episode_reward = np.zeros(len(self.rewards), dtype=np.float32)
    ...
    def step(self, action):
        ...
        # Compute vectorized step reward for MORL
        step_reward_vector = np.zeros(len(self.rewards), dtype=np.float32)

        for i, reward in enumerate(self.rewards):
            step_reward_vector[i] = reward.get_reward()
        ...
        """
        # Accumulate vectorized reward for the episode
        self.episode_reward += step_reward_vector
        
        #print("total: ", step_reward)
        
        return agent_state, step_reward_vector, bool(self.episode_done), {}
