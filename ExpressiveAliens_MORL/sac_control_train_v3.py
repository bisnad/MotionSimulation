...
def randomise_rewards():
    ...
    # 3. Sample continuous preference weights for the objectives (Dirichlet distribution)
    # The length of this vector MUST match your number of objectives (len(env.rewards))
    num_objectives = len(env.rewards)
    
    # We use a uniform Dirichlet distribution to sample preference weights summing to 1.0
    w = np.random.dirichlet(np.ones(num_objectives)).astype(np.float32)

    # 4. Assign the new scales and targets to the reward objects
    # Note: the scales aren't strictly needed for the reward calculation itself anymore 
    # since SAC scalarizes dynamically, but you can leave them for metric tracking.
    
    # Sample discrete polarities (0.0 or 1.0) for Laban Effort Factors targets
    ...
    # Overwrite controlState targets (do not overwrite scales, w handles that)
...
def export_episode(env, sim_file, reward_file, value_file):
    ...
    while not(d or (ep_len == max_ep_len)):
        # Pass in weights
        a = sac.get_action(np.expand_dims(o, 0), np.expand_dims(w, 0))
        a = np.squeeze(a, 0)

        # Step the env (receives vector)
    ...
    # (The plotting block below remains identical)
...
def train_agent(epochs, steps_per_epoch, render):
    ...
    ep_ret = 0.0 # Keep tracking the scalarized return for logging
    ...
    for t in range(total_steps):
        ...
        # Calculate scalarized reward for tracking progress
        r_scalar = np.dot(r_vec, w)
        ep_ret += r_scalar
        ep_len += 1

        ep_reward[0] += r_scalar
        for rI, rew_val in enumerate(r_vec):
            ep_reward[rI+1] += rew_val
        ...
        # Store vectorized experience to replay buffer
        sac.store_experience(o, a, r_vec, o2, d)
        ...
        if d or (ep_len == max_ep_len):
            ...
            for rI, reward_name in enumerate(env.reward_names):
            ...
            # RESAMPLE PREFERENCES w for the next episode
            randomise_target_pos()
            w = randomise_rewards()
            o, d, ep_len = env.reset(), False, 0
            ep_ret = 0.0
